...
const handleNavbarVerticalCollapsed = () => {
  ...
  if (navbarVerticalToggle) {
    navbarVerticalToggle.addEventListener(Events.CLICK, (e) => {
      ...
      setItemToStore('isNavbarVerticalCollapsed', String(!isNavbarVerticalCollapsed))
  ...
  if (navbarVerticalCollapse) {
    navbarVerticalCollapse.addEventListener(Events.MOUSE_OVER, () => {
      if (hasClass(html as HTMLElement, ClassNames.NAVBAR_VERTICAL_COLLAPSED)) {
        html?.classList.add(ClassNames.NAVBAR_VERTICAL_COLLAPSED_HOVER)
      }
    })
    navbarVerticalCollapse.addEventListener(Events.MOUSE_LEAVE, () => {
      if (hasClass(html as HTMLElement, ClassNames.NAVBAR_VERTICAL_COLLAPSED_HOVER)) {
