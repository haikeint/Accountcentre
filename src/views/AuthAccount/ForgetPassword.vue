...
const validFn = {
  ...
  checkEmail: (event: IInputEvent) => {
    validState.emailClass = event.target.value
      ? 'form-control valid-txt'
      : 'form-control invalid-txt'
  }
}
...
const checkValiation = () => {
  validState.usernameClass = !forgetPWState.username
    ? 'form-control invalid-txt'
    : 'form-control valid-txt'
  validState.emailClass = !forgetPWState.email
    ? 'form-control invalid-txt'
    : 'form-control valid-txt'
...
const btnRefresh = (event: IInputEvent, isLoading: boolean, message: string) => {
  ...
  event.target.innerHTML = isLoading ? '<i class="fas fa-spinner fa-spin"></i> Đang xử lý' : 'Xử lý'
}
                ...
                <div class="mb-3">
                  <InputText
                    ...
                    :events="{
                      ...
                      change: (event: IInputEvent) => (forgetPWState.username = event.target.value)
                    }"
                  />
                </div>
                <div class="mb-3">
                  <InputText
                    ...
                    :events="{
                      ...
                      change: (event: IInputEvent) => (forgetPWState.email = event.target.value)
                    }"
                  />
                </div>
                <ButtonView
